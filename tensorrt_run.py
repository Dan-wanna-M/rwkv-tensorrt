...
class InterOp():
    ...
    def forward(self, input_token, input_state):
        input_token = np.array([input_token], dtype=io_tensors["input_token"]["dtype"])
        cuda.memcpy_htod(io_tensors["input_token"]['gpu_data'], input_token)
        ...
        cuda.memcpy_dtoh(self.output_state, io_tensors["output_state"]['gpu_data'])
        return self.logits, self.output_state
...
print(timeit.timeit(lambda: generate(state), number=10)/10)